/**
 * Analytics Service - In-memory analytics tracking
 * Tracks searches, views, and generates trend data
 */
...
interface PublicationTrendYear {
  year: number;
  publications: number;
}
...
const trendingTopics = new Map<string, { count: number; lastSearched: string }>();
...
export const trackSearch = async (data: {
  ...
  const current = trendingTopics.get(normalizedQuery) || {
    count: 0,
    lastSearched: '',
  };
...
export const trackCardView = async (data: {
  ...
  query: string;
  sessionId?: string;
}): Promise<CardViewEvent> => {
  const event: CardViewEvent = {
    id: uuidv4(),
    type: 'card_view',
    cardId: data.cardId,
    query: data.query,
...
export const getAnalyticsSummary = async (timeRange = '7d') => {
  ...
  // Daily breakdown
  const dailyStats: Record<string, { searches: number; sessions: Set<string> }> = {};
  recentSearches.forEach((s) => {
 ...
 * Get trending topics
 */
export const getTrendingTopics = async (limit = 10): Promise<TrendingTopic[]> => {
 ...
 */
export const getPublicationTrend = async (query: string): Promise<PublicationTrend> => {
  const currentYear = new Date().getFullYear();
  const years: PublicationTrendYear[] = [];
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const value = Math.floor(
      baseValue * Math.pow(1 + growthRate, 5 - i) * (0.9 + Math.random() * 0.2)
    );
