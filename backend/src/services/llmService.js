 ...
 */
export const generateTrendSummary = async (query, documents) => {
...
Ensure all claims are grounded in the provided sources. Do not make up information not present in the documents.`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert market research analyst specializing in trend analysis. Always respond with valid JSON. Be concise, factual, and cite sources using [n] format.'
        },
        {
          role: 'user',
          content: prompt
 ...
 */
export const generateSubTopics = async (query, documents) => {
  if (!openai) {
    return generateMockSubTopics(query, documents);
  }
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert at categorizing and organizing research topics. Respond with valid JSON only.'
        },
        {
          role: 'user',
...
Return JSON:
{
  "subTopics": [
    ...
    console.error('Error generating sub-topics:', error.message);
    return generateMockSubTopics(query, documents);
  }
};
 ...
 */
const generateMockSummary = (query, documents) => {
 ...
 */
const generateMockSubTopics = (query, documents) => {
  return {
    subTopics: [
      {
